...
import { div, h1, table } from "framer-motion/client";
...
export default function StudentSearch() {
  ...
  const handleSearch = () => {
    ...
    setTimeout(() => {
      ...
      if (term.trim() === '') {
        // show a message instead of filtering or set filtered to empty array
        setFiltered([]);
        setLoading(false);
        setSearched(true);
        return ; // exit early
      ...
      const results = students.filter(
        (s) => {
          // if term length is 2 serch by roll
          if ((term.length === 1) || (term.length === 2)) {
            return s.roll === Number(term);
          }

          else if (term.length == 3) {
            return s.id.toLowerCase().endsWith(term);
          }
          else {
      ...
      <div className="container">
        ...
        {!loading && searched && (
          filtered.length > 0 ? (
            ...
            <div className="no-results">No students found, Please check your Roll or Id number.</div>
          )
        )}
      </div>
